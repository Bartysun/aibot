...
async def start(update, context):
    ...
    await show_main_menu(update, context, {
        ...
        "opener": "Сообщения для знакомства 🥰",
...
async def profile_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        await send_text(update, context, "Где вы работаете?")
    elif dialog.count == 2:
...
async def opener_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        dialog.user["age"] = text
        await send_text(update, context, "Оцените ее внешност: 1-10 баллов??")
    elif dialog.count == 3:
        ...
        my_message = await send_text(update, context, "ChatGPT генерирует сообщение...")
        answer = await chatgpt.send_question(prompt, user_info)
        await my_message.edit_text(answer)
